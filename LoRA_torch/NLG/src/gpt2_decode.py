...
if __name__ == "__main__":
    ...
    log_dir_name = f'decode_{current_time}'
    log_dir = os.path.join(args.log_dir, log_dir_name)
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    logging.basicConfig(filename=os.path.join(log_dir, "decode.log"), level=logging.INFO,
    ...
    if args.ref_unique_file is not None:
        ...
        logging.info('reading ref_unique_file.')
        ref_unique = []
        uniques = {}
        ...
        logging.info(f'len refer dict {len(ref_unique)} unique {len(uniques)}')
    ...
    with open(args.sample_file, 'r') as sample_reader, \
        ...
        print(f'unique refer dict {len(refer_dict)}')
        logging.info(f'unique refer dict {len(refer_dict)}')
