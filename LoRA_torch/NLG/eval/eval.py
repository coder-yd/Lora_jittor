...
def meteor_score(references, hypothesis, num_refs, lng='en'):
    ...
    for i in range(len(references)):
        if hypothesis[i].strip() and all(ref.strip() for ref in references[i]):
...
def run(refs_path, hyps_path, num_refs, lng='en', metrics='bleu,meteor,chrf++,ter,bert,bleurt', ncorder=6, nworder=2,
        beta=2):
...
if __name__ == '__main__':
    ...
    argParser.add_argument("-nc", "--ncorder", help="chrF metric: character n-gram order (default=6)", type=int,
                           default=6)
    ...
    result = run(refs_path=refs_path, hyps_path=hyps_path, num_refs=num_refs, lng=lng, metrics=metrics, ncorder=ncorder,
                 nworder=nworder, beta=beta)
