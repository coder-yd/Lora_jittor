...
from jittor.optim import LambdaLR, LRScheduler, AdamW
...
#         assert 0.0 <= betas[0] < 1.0, "Invalid beta parameter: {}".format(betas[0])
#         assert 0.0 <= betas[1] < 1.0, "Invalid beta parameter: {}".format(betas[1])
...
#             pg.update(defaults)
#             pg["exp_avg"] = [jt.zeros_like(p) for p in pg["params"]]
#             pg["exp_avg_sq"] = [jt.zeros_like(p) for p in pg["params"]]
...
#             for p, g, exp_avg, exp_avg_sq in zip(pg["params"], pg["grads"],  pg["exp_avg"], pg["exp_avg_sq"]):
#                 if p.is_stop_grad(): continue
#                 p.update(p * (1 - lr * weight_decay))
#                 bias_correction1 = 1 - b0 ** n
#                 bias_correction2 = 1 - b1 ** n

#                 exp_avg.update(b0 * exp_avg + (1 - b0) * g)
#                 exp_avg_sq.update(b1 * exp_avg_sq + (1 - b1) * g * g)

#                 denom = jt.sqrt(exp_avg_sq) / jt.sqrt(bias_correction2) + eps
#                 step_size = lr / bias_correction1
#                 # 更新参数
#                 denom = jt.sqrt(exp_avg_sq) / math.sqrt(bias_correction2) + eps
#                 p.update(p - step_size * exp_avg / denom)

#                 # 处理权重衰减
#                 if weight_decay > 0.0:
#                     p.update(p - lr * weight_decay * p)
