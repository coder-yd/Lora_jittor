...
def _reorder_cache(past: Tuple, beam_idx: Var) -> Tuple[Any, ...]:
    # return tuple(jt.contiguous(jt.index_select(layer_past,1,beam_idx)).detach() for layer_past in past)
    #past (list 12)(2,10,12,256,64)
    #beam_idx(10)
    # 此处index_select的结果与说明文档不符合 改变了layer_past 的形状
...
def _enforce_repetition_penalty_(
    ...
    for i in range(batch_size * num_beams):
        ...
        logging.info(f'prev_output_tokens.shape {prev_output_tokens.shape}')
        print('prev_output_tokens[i].shape', prev_output_tokens[i].shape)
        logging.info(f'prev_output_tokens[i].shape {prev_output_tokens[i].shape}')
...
def _add_beam_candidate(
    ...
    for _i in range(batch_size * num_beams):
        a = last_tokens[_i].data.item()
        b = eos_token_id
        if eos_token_id is None or last_tokens[_i].data.item() in eos_token_id:
...
def beam(model, data_iter, args):
    ...
    with jt.no_grad():
        for idx, data in enumerate(data_iter):
            ...
            _batch = jt.arange(0, _id.size(0), dtype="int32")
            ...
            best_sequence = jt.zeros(
                (batch_size, args.eval_len), dtype="int32"
            )
            best_score = {}
...
if __name__ == '__main__':
    ...
    log_dir_name = f'beam_{current_time}'
